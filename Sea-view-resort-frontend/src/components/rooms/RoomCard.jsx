...
export default function RoomCard({
  ...
  const currentImage =
    images[currentIndex] || images[0] || "https://via.placeholder.com/400";
    ...
    <Card className="group overflow-hidden bg-white rounded-xl border border-gray-200/60 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
      ...
      >
        <img
          src={currentImage}
          alt={title}
          className="w-full h-56 md:h-64 lg:h-72 object-cover transition-transform duration-500 group-hover:scale-[1.03]"
        />
        ...
        <div className="absolute top-3 left-3 flex items-center gap-2">
          <span
            className={`px-2.5 py-1 rounded-full text-xs font-medium backdrop-blur bg-white/80 ${available === "Yes" ? "text-emerald-700" : "text-rose-700"}`}
          >
            {available === "Yes" ? "Disponible" : "No disponible"}
          </span>
        </div>
        {images.length > 1 && (
          <>
